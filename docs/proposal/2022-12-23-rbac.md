...
To ensure no plugin has access to the sensitive configuration data, we will modify the plugin manager to run command with different user and/or group id.
Another option is to use Chroot, however that will require root access or some root-less workarounds, and possibly copying/symlinking some system directories to ensure the GRPC server can run.
...
Username and groups prefix could be useful in the clusters where OIDC is configured. For example: https://fusionauth.io/blog/2022/02/24/rbac-with-kubernetes-fusionauth
  ...
  'kubectl-read-only':
    botkube/kubectl:
      ...
      config:
        ...
        # Different mappings for multiple executor bindings are not supported?
