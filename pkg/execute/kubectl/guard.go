...
var (
	...
	additionalResourcelessVerbs = map[string][]string{
		"pods": {"logs"},
	}
	// notSupportedGlobalVerbs contains verbs returned by K8s API which are not supported for event-related operations.
	notSupportedGlobalVerbs = []string{"create", "update", "list", "patch", "watch", "deletecollection"}
)
...
func (g *CommandGuard) GetAllowedResourcesForVerb(selectedVerb string, allConfiguredResources []string) ([]Resource, error) {
	...
	for _, configuredRes := range allConfiguredResources {
		res, err := g.GetResourceDetailsFromMap(selectedVerb, configuredRes, resMap)
		if err != nil {
...
func (g *CommandGuard) GetResourceDetailsFromMap(selectedVerb, resourceType string, resMap map[string]v1.APIResource) (Resource, error) {
	...
	addReslessVerbs, exists := additionalResourcelessVerbs[resourceType]
	if exists && slices.Contains(addReslessVerbs, selectedVerb) {
		return Resource{
			Name:                    res.Name,
			Namespaced:              res.Namespaced,
...
func (g *CommandGuard) getAllSupportedVerbs(resourceType string, inVerbs v1.Verbs) v1.Verbs {
	...
	verbs = slices.Filter(nil, verbs, func(s string) bool {
		return !slices.Contains(notSupportedGlobalVerbs, s)
	})
